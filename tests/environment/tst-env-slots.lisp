...
(defclass env-slots-tests (test-case)
  ((env :accessor env)
   (tmpl :accessor tmpl :initform (make-template :tmpl '(a (b :default 5))))))

(defmethod set-up ((tests env-slots-tests))
  (with-slots (env) tests
    (setf env (make-environment))))

(def-test-method test-watchers ((tests env-slots-tests) :run nil)
  (with-slots (env) tests
...
(def-test-method test-templates ((tests env-slots-tests) :run nil)
...
(def-test-method test-simple-facts ((tests env-slots-tests) :run nil)
  (with-slots (env) tests
...
(def-test-method test-tmpl-facts ((tests env-slots-tests) :run nil)
  (with-slots (env tmpl) tests
...
(def-test-method test-fact-groups ((tests env-slots-tests) :run nil)
  (with-slots (env) tests
...
(def-test-method test-rules ((tests env-slots-tests) :run nil)
  (with-slots (env) tests
...
(add-test-suite 'env-slots-tests)
;(textui-test-run (get-suite env-slots-tests))
