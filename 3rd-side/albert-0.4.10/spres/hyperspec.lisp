...
DESC: spres/hyperspec.lisp - links to the hyperspec
...
(defun link-to-clhs? (name)
  "Returns either NIL or a link to the CLHS."
  (when (stringp name)
    (gethash (string-downcase name) *clhs-table*)))

(defgeneric clhs-link (doc name)
  (:documentation "Returns the name if there is no CLHS-link, or a CLHS-link."))

(defmethod clhs-link (doc name)
  (error "No clhs-link implemented for (~a,~a)" doc name))

(defun register-clhs-root (root)
	...
	'(("&allow-other-keys" "03_da.htm")
	  ("&aux" "03_da.htm")
	    ...
	    ("(setf get)" "f_get.htm")
            ...
            ("getf" "f_getf.htm")
	    ("(setf getf)" "f_getf.htm")
            ("gethash" "f_gethas.htm")
	    ("(setf gethash)" "f_gethas.htm")
            ("go" "s_go.htm")
	    ...
	    ("(setf slot-value)" "f_slt_va.htm")
  	    ...
  	    ("(setf svref)" "f_svref.htm")
            ...
            ("zerop" "f_zerop.htm")))
  ;; add all to table
  (push (cadr i) (gethash (car i) *clhs-table*)))
